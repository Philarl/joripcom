...
CREATE TABLE CART_TBL(
    ...
    FOREIGN KEY (P_NO) REFERENCES P_TBL (P_NO) ON DELETE CASCADE,
    FOREIGN KEY (U_ID) REFERENCES U_TBL (U_ID) ON DELETE CASCADE
);
CREATE SEQUENCE seq_cart_no;

-- 장바구니 백업 테이블
DROP TABLE BACKUP_CART_TBL CASCADE CONSTRAINTS;
CREATE TABLE BACKUP_CART_TBL(
    CART_NO         NUMBER,
    P_NO            NUMBER,
    U_ID            VARCHAR2(20),
    CART_AMT        NUMBER,
    BACKUP_CART_DATE DATE DEFAULT SYSDATE
);
-- 장바구니 백업 트리거
CREATE OR REPLACE TRIGGER tr_backup_cart_tbl
AFTER DELETE ON cart_tbl
FOR EACH ROW
    BEGIN
    IF DELETING THEN
        INSERT INTO backup_cart_tbl(cart_no, p_no, u_id, cart_amt)
        VALUES (:OLD.cart_no, :OLD.p_no, :OLD.u_id, :OLD.cart_amt);
    END IF;
END tr_backup_cart_tbl;
...
-- 찜 목록 백업 테이블
DROP TABLE BACKUP_FAV_TBL CASCADE CONSTRAINTS;
CREATE TABLE BACKUP_FAV_TBL(
    FAV_NO  NUMBER,
    P_NO    NUMBER,
    U_ID    VARCHAR2(20),
    BACKUP_FAV_DATE DATE DEFAULT SYSDATE
);

-- 찜 목록 백업 트리거
CREATE OR REPLACE TRIGGER tr_backup_fav_tbl
AFTER DELETE ON fav_tbl
FOR EACH ROW
    BEGIN
    IF DELETING THEN
        INSERT INTO backup_fav_tbl(fav_no, p_no, u_id)
        VALUES (:OLD.fav_no, :OLD.p_no, :OLD.u_id);
    END IF;
END tr_backup_fav_tbl;
...
-- 최근 본 상품 백업 테이블
DROP TABLE BACKUP_RW_TBL CASCADE CONSTRAINTS;
CREATE TABLE BACKUP_RW_TBL(
    RW_NO   NUMBER,
    P_NO    NUMBER,
    U_ID    VARCHAR2(20),
    BACKUP_RW_DATE DATE DEFAULT SYSDATE
);

-- 최근 본 상품 백업 트리거
CREATE OR REPLACE TRIGGER tr_backup_rw_tbl
AFTER DELETE ON rw_tbl
FOR EACH ROW
    BEGIN
    IF DELETING THEN
        INSERT INTO backup_rw_tbl(rw_no, p_no, u_id)
        VALUES (:OLD.rw_no, :OLD.p_no, :OLD.u_id);
    END IF;
END tr_backup_rw_tbl;
...
-- 결제 백업 테이블
DROP TABLE BACKUP_ORD_TBL CASCADE CONSTRAINTS;
CREATE TABLE BACKUP_ORD_TBL(
    ORD_NO              NUMBER,
    U_ID                VARCHAR2(20),
    ORD_NAME            VARCHAR2(30)    NOT NULL,
    ORD_ZIPCODE         CHAR(5)         NOT NULL,
    ...
    ORD_SUM             NUMBER          NOT NULL,
    PAY_METHOD          VARCHAR2(15)    NOT NULL,
    BACKUP_ORDER_DATE          DATE            DEFAULT SYSDATE NOT NULL
);

-- 결제 백업 트리거
CREATE OR REPLACE TRIGGER tr_backup_ord_tbl
AFTER DELETE ON ord_tbl
FOR EACH ROW
    BEGIN
    IF DELETING THEN
        INSERT INTO backup_ord_tbl(ord_no, u_id, ord_name, ord_zipcode, ord_addr, ord_addr_dtl, ord_phone, ord_email, ord_sum, pay_method)
        VALUES (:OLD.ord_no, :OLD.u_id, :OLD.ord_name, :OLD.ord_zipcode, :OLD.ord_addr, :OLD.ord_addr_dtl, :OLD.ord_phone, :OLD.ord_email, :OLD.ord_sum, :OLD.pay_method);
    END IF;
END tr_backup_ord_tbl;
...
CREATE TABLE OD_TBL(
    ...
    FOREIGN KEY (ORD_NO) REFERENCES ORD_TBL (ORD_NO) ON DELETE CASCADE,
    FOREIGN KEY (P_NO) REFERENCES P_TBL (P_NO) ON DELETE CASCADE
);
...
-- 결제 백업 테이블
DROP TABLE BACKUP_PAY_TBL CASCADE CONSTRAINTS;
CREATE TABLE BACKUP_PAY_TBL(
    PAY_NO      NUMBER,
    ORD_NO      NUMBER,
    U_ID        VARCHAR2(20),
    PAY_METHOD  VARCHAR2(15)    NOT NULL,
    ...
    PAY_BANK    VARCHAR2(15)    NOT NULL,
    PAY_MEMO    VARCHAR2(100)   NOT NULL,
    BACKUP_PAY_DATE    DATE DEFAULT SYSDATE
);

-- 결제 백업 트리거
CREATE OR REPLACE TRIGGER tr_backup_pay_tbl
AFTER DELETE ON pay_tbl
FOR EACH ROW
    BEGIN
    IF DELETING THEN
        INSERT INTO backup_pay_tbl(pay_no, ord_no, u_id, pay_method, pay_px, pay_user, pay_bank, pay_memo)
        VALUES (:OLD.pay_no, :OLD.ord_no, :OLD.u_id, :OLD.pay_method, :OLD.pay_px, :OLD.pay_user, :OLD.pay_bank, :OLD.pay_memo);
    END IF;
END tr_backup_pay_tbl;
...
CREATE TABLE REV_TBL (
    ...
    FOREIGN KEY (U_NIC) REFERENCES U_TBL (U_NIC) ON DELETE CASCADE,
    FOREIGN KEY (P_NO) REFERENCES P_TBL (P_NO) ON DELETE CASCADE
);
...
CREATE TABLE CSTG_TBL (
    ...
    FOREIGN KEY (U_ID) REFERENCES U_TBL (U_ID) ON DELETE CASCADE,
    FOREIGN KEY (CPN_NO) REFERENCES CPN_TBL (CPN_NO) ON DELETE CASCADE
);
