package com.joripcom.interceptor;
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(loginAuth == null) {
			...
			if(isAjaxRequest(request)) {
				...
				// ajax 요청은 로그인 jsp를 사용불가능. 이유는 리턴값만 부분업데이트 성격으로 받는 특징 때문에
//				response.sendRedirect("/member/login");
	...
	private void getTargetUrl(HttpServletRequest request) {
		...
		// localhost:8888/product/pro_list?pro_num=10
		String uri = request.getRequestURI(); // localhost:8888/product/pro_list
